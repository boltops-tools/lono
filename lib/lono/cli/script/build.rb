  ...
  class Build < Base
    ...
    def run
      ...
      if Dir["#{@blueprint.root}/scripts/*"].empty?
        ...
        puts "Detected scripts"
      end

      puts "Tarballing scripts folder to scripts.tgz"
    ...
    def create_tarball
      ...
      system "cd #{@blueprint.root}/app && dot_clean ." if system("type dot_clean > /dev/null 2>&1")
      ...
      sh "cd #{@blueprint.root}/app && tar -c scripts | gzip -n > scripts.tgz" # temporary scripts.tgz file
